...
module MIPSRun
  ...
  def compile(s)
    ...
    c.scan srcfile
    # c.asm_header("src/gas/mips/#{model}/const.inc")
    c.compile
    c.image imagefile
    imagefile
  end
  ...
  def gdbtalk(gdb, tgt)
    ...
    n += 1
    gdb.send("#{n}-data-read-memory &_spsave x #{@size} 1 1")
    rsp = gdb.receive
    sp1 = rsp.result.value['memory'][0]['data'][0]
  ...
  def qemurun(imagefile)
    ...
    @qemu_mips = ENV[@qemu_env] || "qemu-#{@arch}"
    @qemu = IO.popen("#{@qemu_mips} -g #{@gdbport} #{imagefile}")
  end

  def mipsrun(imagefile)
    tgt = nil
